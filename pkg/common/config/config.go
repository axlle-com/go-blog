...
func (c *config) DBUrl() string {
	...
	if c.dialector == "postgres" {
		dsn = "host=" + c.dbHost + " user=" + c.dbUser + " password=" + c.dbPassword + " dbname=" + c.dbName + " port=" + c.dbPort + " sslmode=disable TimeZone=Europe/Moscow"
	}
	return dsn
}
...
func (c *config) DBUrlTest() string {
	var dsn string
	if c.dialector == "postgres" {
		dsn = "host=" + c.dbHost + " user=" + c.dbUserTest + " password=" + c.dbPasswordTest + " dbname=" + c.dbNameTest + " port=" + c.dbPort + " sslmode=disable TimeZone=Europe/Moscow"
	}
	return dsn
}
...
func (c *config) UploadPath() string {
	if c.IsTest() {
		if c.uploadsPath == "" {
			c.uploadsPath = "/public/uploads/test/"
		}
		return c.uploadsPath
	}
	if c.uploadsPath == "" {
		c.uploadsPath = "/public/uploads/"
	}
	return c.uploadsPath
}
